...
function Invoke-PsqlCsv {
    ...
    while ($queryText.EndsWith(';')) { $queryText = $queryText.Substring(0,$queryText.Length-1).TrimEnd() }
    return Invoke-PsqlText -PsqlExe $PsqlExe -Database $Database -Sql "COPY (`n$queryText`n) TO STDOUT WITH (FORMAT CSV, HEADER TRUE);"
}
...
function Get-NominalSlotCount {
    return [int](($ContractEndExclusiveUtc - $ContractStartUtc).TotalMinutes / $CadenceMinutes)
}

function Get-ObjectKey {
    param([datetimeoffset]$Timestamp)
    return $Timestamp.UtcDateTime.ToString('yyyyMMddHHmmss',[Globalization.CultureInfo]::InvariantCulture)
}

function Get-SecureUrl {
    param([string]$ObjectKey)
    return $UrlTemplate.Replace('{object_key}',$ObjectKey)
}
...
function Get-ProviderMd5Base64 {
    ...
    if ($null -ne $Response.Content.Headers.ContentMD5) {
        return [Convert]::ToBase64String([byte[]]$Response.Content.Headers.ContentMD5)
    }
    return ''
}
...
function Ensure-SourceSchema {
    param([string]$PsqlExe,[string]$ContractSha,[string]$ArchiveRootPath,[string]$GitCommit)
    $slotCount = Get-NominalSlotCount
    $gitSql = Sql-Literal $GitCommit
    $ddl = @"
...
INSERT INTO source_news.source_contracts(
    contract_sha256,source_product,interval_start_utc,interval_end_exclusive_utc,cadence_minutes,nominal_slot_count,url_template,created_by_git_commit
) VALUES (
    $(Sql-Literal $ContractSha),$(Sql-Literal $SourceProduct),TIMESTAMPTZ '2025-04-01 00:00:00+00',TIMESTAMPTZ '2025-07-01 00:00:00+00',$CadenceMinutes,$slotCount,$(Sql-Literal $UrlTemplate),$gitSql
)
...
SELECT
    $(Sql-Literal $ContractSha),
    to_char(gs AT TIME ZONE 'UTC','YYYYMMDDHH24MISS'),
    gs,
    replace($(Sql-Literal $UrlTemplate),'{object_key}',to_char(gs AT TIME ZONE 'UTC','YYYYMMDDHH24MISS')),
    'pending'
FROM generate_series(
    TIMESTAMPTZ '2025-04-01 00:00:00+00',
    TIMESTAMPTZ '2025-07-01 00:00:00+00' - interval '15 minutes',
    interval '15 minutes'
) AS gs
...
LIMIT $BatchSize
"@
    $csv = Invoke-PsqlCsv -PsqlExe $PsqlExe -Database $DatabaseName -Query $query
...
function Invoke-ObjectDownload {
    ...
    $year = $ObjectKey.Substring(0,4)
    $month = $ObjectKey.Substring(4,2)
    $day = $ObjectKey.Substring(6,2)
    $relative = ('archives/' + $year + '/' + $month + '/' + $day + '/' + $ObjectKey + '.gkg.csv.zip')
    ...
    for ($httpAttempt=1; $httpAttempt -le $HttpAttemptsPerObject; $httpAttempt++) {
        ...
        try {
            ...
            if ($statusCode -eq 404) {
                return [pscustomobject]@{ object_key=$ObjectKey; status='provider_missing'; http_status=404; expected_content_length=$null; observed_size_bytes=$null; payload_sha256=$null; provider_md5_base64=$null; observed_md5_base64=$null; provider_md5_status=$null; local_relative_path=$null; zip_entry_count=$null; error_code=$null }
            }
            ...
            if ($null -ne $expectedLength -and [long]$expectedLength -ne $size) {
                throw "Content-Length mismatch expected=$expectedLength observed=$size"
            }
            ...
            return [pscustomobject]@{ object_key=$ObjectKey; status='downloaded'; http_status=$statusCode; expected_content_length=if($null -eq $expectedLength){$null}else{[long]$expectedLength}; observed_size_bytes=$size; payload_sha256=$sha256; provider_md5_base64=if([string]::IsNullOrWhiteSpace($providerMd5)){$null}else{$providerMd5}; observed_md5_base64=$observedMd5; provider_md5_status=$md5Status; local_relative_path=$relative; zip_entry_count=$entryCount; error_code=$null }
        }
        catch {
            Remove-Item -LiteralPath $tempPath -Force -ErrorAction SilentlyContinue
            if ($httpAttempt -ge $HttpAttemptsPerObject) {
                return [pscustomobject]@{ object_key=$ObjectKey; status='network_failed'; http_status=if($null -eq $response){$null}else{[int]$response.StatusCode}; expected_content_length=$null; observed_size_bytes=$null; payload_sha256=$null; provider_md5_base64=$null; observed_md5_base64=$null; provider_md5_status=$null; local_relative_path=$null; zip_entry_count=$null; error_code=(Get-ErrorCode -Message $_.Exception.Message) }
            }
...
function Save-BatchResults {
    ...
    foreach ($r in $Results) {
        ...
        [void]$sql.AppendLine(@"
UPDATE source_news.source_slots
SET status=$(Sql-Literal $r.status),
    attempt_count=attempt_count+1,
    http_status=$http,
    expected_content_length=$expected,
    observed_size_bytes=$observed,
    payload_sha256=$(Sql-Literal $r.payload_sha256),
    provider_md5_base64=$(Sql-Literal $r.provider_md5_base64),
    observed_md5_base64=$(Sql-Literal $r.observed_md5_base64),
    provider_md5_status=$(Sql-Literal $r.provider_md5_status),
    local_relative_path=$(Sql-Literal $r.local_relative_path),
    zip_entry_count=$zipCount,
    last_attempt_at_utc=clock_timestamp(),
    error_code=$(Sql-Literal $r.error_code)
WHERE contract_sha256=$(Sql-Literal $ContractSha)
  AND object_key=$(Sql-Literal $r.object_key);
...
function Invoke-SelfTest {
    if ((Get-NominalSlotCount) -ne 8736) { throw 'Self-test failed: Q2 nominal slot count is not 8736.' }
    ...
    if ((Convert-HexToBase64 -Hex 'd41d8cd98f00b204e9800998ecf8427e') -ne '1B2M2Y8AsgTpgAmY7PhCfg==') { throw 'Self-test failed: MD5 hex conversion.' }
    ...
    try {
        ...
        if ((Test-ZipStructure -Path $zipPath) -ne 1) { throw 'Self-test failed: ZIP structure validation.' }
...
try {
    ...
    $psql = Find-Psql
    $contractSha = Get-ContractSha256
    ...
    Ensure-SourceSchema -PsqlExe $psql -ContractSha $contractSha -ArchiveRootPath $ArchiveRoot -GitCommit $gitCommit
...
SELECT status,count(*)::bigint AS exact_slots
FROM source_news.source_slots
WHERE contract_sha256=$(Sql-Literal $contractSha)
GROUP BY status
ORDER BY status
"@
    $summary = @($summaryCsv | ConvertFrom-Csv)
    $unresolved = 0L
    foreach ($row in $summary) {
        if ([string]$row.status -in @('pending','network_failed','integrity_failed')) { $unresolved += [long]$row.exact_slots }
    }
    ...
    if ($runInserted) {
        try {
            $psqlForFailure = Find-Psql
            [void](Invoke-PsqlText -PsqlExe $psqlForFailure -Database $DatabaseName -Sql ("UPDATE source_news.acquisition_runs SET status='failed',completed_at_utc=clock_timestamp() WHERE run_id=" + (Sql-Literal $runId.ToString()) + ';'))
        }
        catch { }
    }
    Write-Host 'CFA GDELT Q2 SOURCE ACQUISITION: FAIL'
