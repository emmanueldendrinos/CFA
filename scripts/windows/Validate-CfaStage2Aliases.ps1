...
function New-AliasState{param([object]$Row)
    return [pscustomobject]@{base_asset_id=[string]$Row.base_asset_id;alias_text=[string]$Row.alias_text;alias_type=[string]$Row.alias_type;requires_crypto_context=(Normalize-Bool $Row.requires_crypto_context);mapping_tier=[string]$Row.mapping_tier;mention_count=0L;document_count=0L;context_supported_document_count=0L;distinct_sources=@{};first_date='';last_date='';samples=New-Object System.Collections.ArrayList}
}
function Invoke-SelfTest{$p=Parse-AllNames 'Bitcoin,10;World Cup,25;ACME,40';if($p.items.Count-ne3-or$p.malformed-ne0){throw 'allnames parser'};if($p.items[1].name-ne'World Cup'-or$p.items[1].offset-ne25){throw 'allnames values'};$bad=Parse-AllNames 'Bitcoin,10;badblock';if($bad.malformed-ne1){throw 'malformed block'};if($AllNamesIndex-ne23-or$ExpectedFieldCount-ne27){throw 'schema constants'};Write-Host 'SELF-TEST: PASS'}
if($SelfTest){try{Invoke-SelfTest;exit 0}catch{Write-Host 'SELF-TEST: FAIL';Write-Host $_.Exception.Message;exit 1}}

try{
    if([string]::IsNullOrWhiteSpace($RepoRoot)){$RepoRoot=[System.IO.Path]::GetFullPath((Join-Path $PSScriptRoot '..\..'))};$RepoRoot=(Resolve-Path -LiteralPath $RepoRoot).ProviderPath;$docs=[Environment]::GetFolderPath('MyDocuments')
    ...
    $archiveRows=@();$sampleRows=@();$totalRows=0L;$malformedFieldRows=0L;$malformedAllNames=0L;$utf8FailureArchives=0;$entryCountFailures=0;$strictUtf8=New-Object System.Text.UTF8Encoding($false,$true);$archiveOrdinal=0
    foreach($file in $files){
        ...
        try{
            $zip=[System.IO.Compression.ZipFile]::OpenRead($file.FullName);$entries=@($zip.Entries|Where-Object{-not[string]::IsNullOrWhiteSpace($_.Name)})
            if($entries.Count-ne1){$entryCountFailures++;$entryStatus='FAIL'}else{
                $stream=$entries[0].Open();$reader=$null
                try{
                    $reader=New-Object System.IO.StreamReader($stream,$strictUtf8,$false,65536,$false)
                    while(($line=$reader.ReadLine()) -ne $null){
                        $rowsThisArchive++;$totalRows++;$fields=$line.Split([char]9)
                        if($fields.Count-ne$ExpectedFieldCount){$malformedFieldRows++;$malformedThisArchive++;continue}
                        $parsed=Parse-AllNames $fields[$AllNamesIndex];$malformedAllNames += [long]$parsed.malformed;if($parsed.items.Count-eq0){continue}
                        $matchedKeys=@{};$presentNames=@{}
                        foreach($item in $parsed.items){$norm=$item.name.ToLowerInvariant();$presentNames[$norm]=$true;if($lookup.ContainsKey($norm)){foreach($aliasKey in $lookup[$norm]){$state=$states[$aliasKey];$state.mention_count=[long]$state.mention_count+1;$matchedKeys[$aliasKey]=$true}}}
                        ...
                        foreach($aliasKey in $matchedKeys.Keys){$state=$states[$aliasKey];$state.document_count=[long]$state.document_count+1;if($state.requires_crypto_context-and$hasContextAnchor){$state.context_supported_document_count=[long]$state.context_supported_document_count+1};$source=[string]$fields[$SourceCommonNameIndex];if(-not[string]::IsNullOrWhiteSpace($source)){$state.distinct_sources[$source]=$true};$date=[string]$fields[$DateIndex];if([string]::IsNullOrWhiteSpace($state.first_date)-or$date-lt$state.first_date){$state.first_date=$date};if([string]::IsNullOrWhiteSpace($state.last_date)-or$date-gt$state.last_date){$state.last_date=$date};if($state.samples.Count-lt$MaxSamplesPerAlias){[void]$state.samples.Add([pscustomobject]@{base_asset_id=$state.base_asset_id;alias_text=$state.alias_text;requires_crypto_context=$state.requires_crypto_context;record_id=[string]$fields[$RecordIdIndex];date_utc=$date;source_common_name=$source;document_identifier=[string]$fields[$DocumentIdentifierIndex];context_anchor_present=$hasContextAnchor})}}
                    }
    ...
    $aliasRows=@()
    foreach($aliasKey in @($states.Keys|Sort-Object)){$s=$states[$aliasKey];$semantic='UNVERIFIED_NOT_OBSERVED';if($s.document_count-gt0){if($s.requires_crypto_context){$semantic='UNVERIFIED_CONTEXT_RULE_REQUIRED'}else{$semantic='OBSERVED_EXACT_PROPER_NAME'}};$aliasRows += [pscustomobject]@{base_asset_id=$s.base_asset_id;alias_text=$s.alias_text;alias_type=$s.alias_type;requires_crypto_context=$s.requires_crypto_context;mapping_tier=$s.mapping_tier;mention_count=$s.mention_count;document_count=$s.document_count;context_supported_document_count=$s.context_supported_document_count;context_unsupported_document_count=([long]$s.document_count-[long]$s.context_supported_document_count);distinct_source_count=$s.distinct_sources.Count;first_date_utc=$s.first_date;last_date_utc=$s.last_date;semantic_observation_status=$semantic};foreach($sample in $s.samples){$sampleRows+=$sample}}
    ...
    $aliasRows|Sort-Object base_asset_id,alias_text|Export-Csv -LiteralPath (Join-Path $dir 'alias-validation.csv') -NoTypeInformation -Encoding UTF8;$sampleRows|Sort-Object base_asset_id,alias_text,date_utc|Export-Csv -LiteralPath (Join-Path $dir 'alias-samples.csv') -NoTypeInformation -Encoding UTF8;$archiveRows|Export-Csv -LiteralPath (Join-Path $dir 'archive-scan.csv') -NoTypeInformation -Encoding UTF8
    $observed=@($aliasRows|Where-Object{[long]$_.document_count-gt0}).Count;$notObserved=45-$observed;$contextRequired=@($aliasRows|Where-Object{[string]$_.requires_crypto_context-eq'True'}).Count
    ...
    Write-Host "Evidence directory: $dir";Write-Host "Rows scanned: $totalRows";Write-Host "Observed aliases: $observed / 45";Write-Host "Malformed field-count rows: $malformedFieldRows";Write-Host "UTF-8 failure archives: $utf8FailureArchives";Write-Host 'CFA STAGE 2 ALIAS VALIDATION: COMPLETE';if($malformedFieldRows-gt0-or$utf8FailureArchives-gt0-or$entryCountFailures-gt0){exit 2}
