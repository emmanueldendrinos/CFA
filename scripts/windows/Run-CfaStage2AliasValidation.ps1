...
function Invoke-AliasScan{
 ...
 if($code-eq0){return 'PASS'}
 if($code-eq2){return 'FAIL'}
 throw "Alias scan execution failed ${code}: $Path"
}
function Invoke-SelfTest{$root=Join-Path([System.IO.Path]::GetTempPath())('cfa-alias-run-'+[guid]::NewGuid().ToString('N'));try{$p=Join-Path $root 'gdelt-alias-validation\20260101-test';New-Item -ItemType Directory -Path $p -Force|Out-Null;[System.IO.File]::WriteAllText((Join-Path $p 'validation-summary.csv'),"archive_files,malformed_field_count_rows,utf8_failure_archives,entry_count_failures`n7163,5,135,0`n");$rows=@();for($i=1;$i-le45;$i++){$rows+=[pscustomobject]@{base_asset_id=('A'+$i)}};$rows|Export-Csv (Join-Path $p 'alias-validation.csv') -NoTypeInformation;foreach($n in @('alias-samples.csv','archive-scan.csv')){[System.IO.File]::WriteAllText((Join-Path $p $n),"x`n")};$r=Get-LatestValidAliasRun (Join-Path $root 'gdelt-alias-validation');if($null-eq$r-or$r.Name-ne'20260101-test'){throw 'reuse detection'};$summary=@(Import-Csv (Join-Path $r.FullName 'validation-summary.csv'));$bad=[int]$summary[0].malformed_field_count_rows+[int]$summary[0].utf8_failure_archives+[int]$summary[0].entry_count_failures;$gate=if($bad-eq0){'PASS'}else{'FAIL'};if($gate-ne'FAIL'){throw 'gate classification'};Write-Host 'SELF-TEST: PASS'}finally{Remove-Item $root -Recurse -Force -ErrorAction SilentlyContinue}}
...
try{
 ...
 Write-Host '';Write-Host '=== CFA STAGE 2 ALIAS EVIDENCE STATUS ===';Write-Host ('Full GKG parser/encoding gate : '+$scanGate);Write-Host 'CFA-S2-005 Alias semantic validation: UNVERIFIED';Write-Host 'Reason: observation/collision evidence is published; source parser/encoding failures must be diagnosed before alias approval, and context-required aliases still require an approved context rule.'
