...
function Assert-Throws {
    ...
    try { & $Script }
        ...
        if ($_.Exception.Message -notmatch $Pattern) {
            throw "Unexpected failure. Expected /$Pattern/; observed: $($_.Exception.Message)"
        }
    }
...
function Assert-HttpsSource {
    ...
    $uri = $null
    if (-not [Uri]::TryCreate($Text,[UriKind]::Absolute,[ref]$uri)) { throw "$Label is not an absolute URL: $Text" }
    if (-not $uri.Scheme.Equals('https',[System.StringComparison]::OrdinalIgnoreCase)) { throw "$Label must use HTTPS: $Text" }
    $host = [string]$uri.Host
    if (-not ($host.Equals($ExpectedHostSuffix,[System.StringComparison]::OrdinalIgnoreCase) -or $host.EndsWith('.' + $ExpectedHostSuffix,[System.StringComparison]::OrdinalIgnoreCase))) {
        throw "$Label host is outside $ExpectedHostSuffix: $host"
    }
}
...
function Get-ExistingAdjudicationBases {
    ...
    foreach ($relative in $paths) {
        ...
        if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw "Existing adjudication registry missing: $relative" }
        foreach ($row in @(Import-Csv -LiteralPath $path)) {
            $base = [string]$row.base_asset_id
            if ([string]::IsNullOrWhiteSpace($base)) { throw "Existing adjudication has an empty base_asset_id: $relative" }
            if ($seen.ContainsKey($base)) { throw "Duplicate existing adjudication base across registries: $base" }
            $seen[$base] = $relative
        }
    }
    return $seen
...
function Assert-CanonicalRegistry {
    ...
    $bases = @{}
    $ids = @{}
    $allowedBasis = @('KRAKEN_PLUS_COINGECKO_CANONICAL_PAGE_ID','KRAKEN_PLUS_COINGECKO_REBRAND_CANONICAL_PAGE_ID')

    foreach ($row in $Registry) {
        foreach ($name in $required) {
            $property = $row.PSObject.Properties[$name]
            if ($null -eq $property -or [string]::IsNullOrWhiteSpace([string]$property.Value)) {
                throw "Canonical adjudication missing required field $name for base $([string]$row.base_asset_id)."
            }
        }

        $base = [string]$row.base_asset_id
        $id = [string]$row.approved_coingecko_id
        if ($bases.ContainsKey($base)) { throw "Duplicate canonical adjudication base: $base" }
        if ($ids.ContainsKey($id)) { throw "Duplicate canonical CoinGecko id across bases: $id" }
        $bases[$base] = $true
        $ids[$id] = $base

        if ([string]$row.decision_status -ne 'APPROVED') { throw "Canonical adjudication must be APPROVED: $base" }
        if ($allowedBasis -notcontains [string]$row.evidence_basis) { throw "Unsupported canonical evidence basis for $base: $([string]$row.evidence_basis)" }
        if ([string]$row.source_observation_date_utc -notmatch '^\d{4}-\d{2}-\d{2}$') { throw "Canonical source observation date must be YYYY-MM-DD: $base" }
        Assert-HttpsSource -Text ([string]$row.coingecko_page_url) -ExpectedHostSuffix 'coingecko.com' -Label "CoinGecko source for $base"
        Assert-HttpsSource -Text ([string]$row.kraken_source_url) -ExpectedHostSuffix 'kraken.com' -Label "Kraken source for $base"

        if ($ExistingBases.ContainsKey($base)) { throw "Canonical adjudication overlaps existing adjudication registry: $base" }

        $decisionMatches = @($Decisions | Where-Object { [string]$_.base_asset_id -eq $base })
        if ($decisionMatches.Count -ne 1) { throw "Expected exactly one current mapping decision for canonical base $base; observed $($decisionMatches.Count)." }
        $decision = $decisionMatches[0]
        if ([string]$decision.mapping_status -ne 'UNVERIFIED') { throw "Canonical adjudication may only replace an UNVERIFIED mapping: $base/$([string]$decision.mapping_status)" }
        if (-not [string]::IsNullOrWhiteSpace([string]$decision.approved_coingecko_id)) { throw "UNVERIFIED canonical target already carries an approved CoinGecko id: $base" }

        $bridgeMatches = @($BridgeRows | Where-Object { [string]$_.base_asset_id -eq $base })
        if ($bridgeMatches.Count -ne 1) { throw "Expected exactly one bridge-evidence row for canonical base $base; observed $($bridgeMatches.Count)." }
        if (-not ([string]$row.observed_kraken_ticker).Equals([string]$bridgeMatches[0].base_exchange_symbol,[System.StringComparison]::OrdinalIgnoreCase)) {
            throw "Canonical Kraken ticker does not match bridge base symbol for $base: observed=$([string]$row.observed_kraken_ticker) bridge=$([string]$bridgeMatches[0].base_exchange_symbol)"
        }
        ...
        if ([string]$row.evidence_basis -eq 'KRAKEN_PLUS_COINGECKO_CANONICAL_PAGE_ID') {
            if (-not ([string]$row.observed_coingecko_symbol).Equals([string]$row.observed_kraken_ticker,[System.StringComparison]::OrdinalIgnoreCase)) {
                throw "Non-rebrand canonical adjudication requires matching CoinGecko/Kraken symbols: $base"
            }
        }
        else {
            if ([string]$row.review_note -notmatch '(?i)rebrand') { throw "Rebrand canonical adjudication must explicitly document rebrand continuity: $base" }
        }
    }
}
...
function Apply-CanonicalMappings {
    ...
    if ($approved -ne ($preApproved + $Registry.Count)) { throw "Canonical approval accounting mismatch: before=$preApproved canonical=$($Registry.Count) after=$approved" }
    if ($unverified -ne ($preUnverified - $Registry.Count)) { throw "Canonical UNVERIFIED accounting mismatch: before=$preUnverified canonical=$($Registry.Count) after=$unverified" }
    if ($na -ne $preNa) { throw "Canonical overlay changed NOT_APPLICABLE count: before=$preNa after=$na" }

    return [pscustomobject]@{rows=@($out | Sort-Object base_asset_id);approved=$approved;unverified=$unverified;not_applicable=$na}
}
...
function Write-DecisionArtifacts {
    ...
    if (-not (Test-Path -LiteralPath $snapshotPath -PathType Leaf)) { throw 'Stage 2 mapping decision snapshot missing before canonical overlay.' }
...
function Invoke-SelfTest {
    ...
    $mapped = @($applied.rows | Where-Object { $_.base_asset_id -eq 'A' })[0]
    ...
    $approvedTarget = @($decisions | ForEach-Object { $_.PSObject.Copy() });(@($approvedTarget | Where-Object {$_.base_asset_id -eq 'A'})[0]).mapping_status='APPROVED'
    ...
    try {
        ...
        Write-Utf8NoBom -Path (Join-Path $root 'docs\evidence\stage2-mapping-decisions.json') -Content (([ordered]@{decision_rows=4;approved=1;not_applicable=1;unverified=2;gate_status='UNVERIFIED';blocking_reason='old'} | ConvertTo-Json) + [Environment]::NewLine)
...
try {
    ...
    foreach ($path in @($decisionPath,$bridgePath,$canonicalPath)) { if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw "Required canonical overlay input missing: $path" } }
    ...
    if ($decisions.Count -ne 435 -or $bridges.Count -ne 435) { throw "Canonical overlay requires 435 decision and bridge rows; observed decisions=$($decisions.Count) bridges=$($bridges.Count)." }
    ...
    Write-Host ('Canonical approvals: ' + $registry.Count)
    Write-Host ('APPROVED: ' + $result.approved)
    Write-Host ('NOT_APPLICABLE: ' + $result.not_applicable)
    Write-Host ('UNVERIFIED: ' + $result.unverified)
    Write-Host ('CFA-S2-003: ' + $(if ($result.unverified -eq 0) {'PASS'} else {'UNVERIFIED'}))
}
catch {
