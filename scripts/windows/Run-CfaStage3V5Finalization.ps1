...
function Invoke-SelfTest {
    foreach ($name in @('Diagnose-CfaStage3CriticalUtf8Impact.ps1','Apply-CfaStage3NewsMatchingV5.ps1','Prepare-CfaStage3V5SampleReview.ps1','Summarize-CfaStage3NewsByAsset.ps1')) {
        if (-not (Test-Path -LiteralPath (Join-Path $PSScriptRoot $name) -PathType Leaf)) { throw "Required V5 component missing: $name" }
    }
    $temp = Join-Path ([IO.Path]::GetTempPath()) ('cfa-s3v5-probe-' + [guid]::NewGuid().ToString('N') + '.ps1')
    try {
...
try {
    ...
    if ([string]::IsNullOrWhiteSpace($OutputRoot)) {
        ...
        $OutputRoot = Join-Path $base ((Get-Date).ToUniversalTime().ToString('yyyyMMdd-HHmmss') + '-v5-finalization-' + [guid]::NewGuid().ToString('N').Substring(0,8))
    }
    ...
    Write-Host ("V5 finalization evidence root: {0}" -f $OutputRoot)

    $impactRoot = Join-Path $OutputRoot 'utf8-impact'
    $v5Root = Join-Path $OutputRoot 'v5'
    ...
    $v5Script = Join-Path $PSScriptRoot 'Apply-CfaStage3NewsMatchingV5.ps1'
    Invoke-ChildScript -Path $v5Script -NamedArguments @{Stage3V2RunRoot=$Stage3V2RunRoot;ImpactRunRoot=$impactRunRoot;RepoRoot=$RepoRoot;ArchiveRoot=$ArchiveRoot;OutputRoot=$v5Root}
    $v5SummaryPath = Join-Path $v5Root 'stage3-match-summary.json'
    $v5 = Get-Content -LiteralPath $v5SummaryPath -Raw | ConvertFrom-Json
    if ([string]$v5.run_status -ne 'PASS' -or [string]$v5.matching_contract -ne 'CANDIDATE_V5') { throw 'V5 short-symbol refinement did not PASS.' }
    if ([string]$v5.gates.'CFA-S3F-016' -ne 'PASS' -or [string]$v5.gates.'CFA-S3F-017' -ne 'PASS' -or [string]$v5.gates.'CFA-S3F-018' -ne 'PASS') { throw 'V5 corrected-candidate gates did not PASS.' }

    $reviewScript = Join-Path $PSScriptRoot 'Prepare-CfaStage3V5SampleReview.ps1'
    Invoke-ChildScript -Path $reviewScript -NamedArguments @{Stage3V5RunRoot=$v5Root;OutputRoot=$reviewRoot}
    $reviewSummary = @(Get-ChildItem -LiteralPath $reviewRoot -Recurse -File -Filter 'stage3-v5-bounded-sample-review-summary.json' | Sort-Object LastWriteTimeUtc -Descending | Select-Object -First 1)
    if ($reviewSummary.Count -ne 1) { throw 'V5 bounded-review summary not found.' }
    $review = Get-Content -LiteralPath $reviewSummary[0].FullName -Raw | ConvertFrom-Json
    if ([string]$review.status -ne 'PASS') { throw 'V5 bounded-review preparation did not PASS.' }

    $countScript = Join-Path $PSScriptRoot 'Summarize-CfaStage3NewsByAsset.ps1'
    Invoke-ChildScript -Path $countScript -NamedArguments @{Stage3RunRoot=$v5Root}
    $countsPath = Join-Path $v5Root 'stage3-news-counts-by-asset.csv'
    if (-not (Test-Path -LiteralPath $countsPath -PathType Leaf)) { throw 'V5 per-asset news count output missing.' }

    $reviewCsv = [string]$review.output_review_csv
    $final = [ordered]@{
        status='VALIDATION_CANDIDATE';stage='CFA_STAGE_3';matching_contract='CANDIDATE_V5';parent_v2_run=$Stage3V2RunRoot;archive_scan_csv=$ArchiveScanCsv;archive_scan_sha256=(Get-Sha $ArchiveScanCsv)
        utf8_impact_summary=$impactSummary[0].FullName;utf8_impact_summary_sha256=(Get-Sha $impactSummary[0].FullName);v5_run_root=$v5Root;v5_summary_sha256=(Get-Sha $v5SummaryPath)
        review_summary=$reviewSummary[0].FullName;review_summary_sha256=(Get-Sha $reviewSummary[0].FullName);review_csv=$reviewCsv;review_csv_sha256=(Get-Sha $reviewCsv);news_counts_csv=$countsPath;news_counts_sha256=(Get-Sha $countsPath)
        ...
        next_action='Directly review every row in review_csv. Stage 3 may freeze only if the V5 bounded review has no obvious false positive or false negative and the review evidence is recorded.'
    }
    $finalPath = Join-Path $OutputRoot 'stage3-v5-finalization-candidate.json'
    Write-Utf8NoBom $finalPath (($final | ConvertTo-Json -Depth 10) + [Environment]::NewLine)

    Write-Host ''
    Write-Host 'CFA STAGE 3 V5 FINALIZATION: VALIDATION CANDIDATE'
    ...
    Write-Host 'CFA-S3F-014 historical V4 semantic review: FAIL (preserved)'
    Write-Host 'CFA-S3F-016 underscore-aware short-symbol title boundary: PASS'
    Write-Host 'CFA-S3F-017 parenthetical market-ticker retention: PASS'
    Write-Host 'CFA-S3F-018 corrected V5 candidate: PASS'
    Write-Host 'CFA-S3F-019 direct V5 semantic review: UNVERIFIED'
    ...
    Write-Host ''
    Write-Host 'CFA STAGE 3 V5 FINALIZATION: FAIL'
