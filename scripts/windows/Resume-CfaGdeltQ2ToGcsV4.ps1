...
function Get-CloudObjects {
    ...
    $r=Invoke-Gcloud $Gcloud @('storage','ls','--recursive','--json',$RawUri) -AllowNonzero
    if($r.ExitCode-ne0){
    ...
    $map=@{}
    foreach($o in $items){
        $url=[string](Get-PropertyValue $o @('url'))
...
function Invoke-SelfTest {
    ...
    try{
        ...
        $r=Invoke-Gcloud $env:ComSpec @('/d','/c','exit','0')
        if($r.ExitCode-ne0 -or $r.Stdout-ne'' -or $r.Stderr-ne''){throw 'Null-safe gcloud wrapper regression failed.'}
...
"@
    ...
    $describe=Invoke-Gcloud $gcloud @('storage','buckets','describe',$bucketUri,'--format=json') -AllowNonzero
    if($describe.ExitCode-ne0){
        Write-Host "Creating GCS bucket: $bucketUri"
        Invoke-Gcloud $gcloud @('storage','buckets','create',$bucketUri,('--location='+$BucketLocation),('--default-storage-class='+$StorageClass),'--uniform-bucket-level-access','--public-access-prevention')|Out-Null
    }else{
    ...
    $pre=Get-CloudObjects $gcloud $rawUri $bucketUri
    Assert-CloudSubsetMatchesManifest $pre $manifest
    Write-Host "Existing matching raw objects before resume: $($pre.Count)"

    Write-Host "Resuming upload to $rawUri"
    Invoke-Gcloud $gcloud @('storage','rsync',$ArchiveRoot,$rawUri,'--recursive')|Out-Null

    Write-Host 'Reconciling final cloud corpus...'
    $cloud=Get-CloudObjects $gcloud $rawUri $bucketUri
    Assert-CloudSubsetMatchesManifest $cloud $manifest -RequireComplete
    ...
    Invoke-Gcloud $gcloud @('storage','cp',$manifestPath,$reconPath,$summaryPath,$manifestUri)|Out-Null
