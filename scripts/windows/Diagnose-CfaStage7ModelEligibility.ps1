...
function Require-Columns {
    ...
    foreach($name in $Names){
        if($props -notcontains $name){throw "$Label required column missing: $name"}
    }
}
...
function Stat-Row {
    ...
    if($count -lt 1){throw "No complete-case observations for $Name"}
    $mean=([double]$Stat.sum)/$count
    $sd=0.0
    if($count -gt 1){
        $numerator=([double]$Stat.sumsq)-($count*$mean*$mean)
        if(($numerator -lt 0) -and ([math]::Abs($numerator) -lt 1e-10)){$numerator=0.0}
        if($numerator -lt 0){throw "Negative sample-variance numerator for $Name"}
        $sd=[math]::Sqrt($numerator/($count-1))
    }
...
function Invoke-SelfTest {
    ...
    Add-Stat $s 3.0 '3'
    $r=Stat-Row 'x' $s
    if(([long]$r.count -ne 3) -or ([int]$r.unique_count -ne 3)){throw 'Stat count self-test failed.'}
    $mean=[double]::Parse([string]$r.mean,$Invariant)
    $sd=[double]::Parse([string]$r.sample_sd,$Invariant)
    if([math]::Abs($mean-2.0) -gt 1e-12){throw 'Stat mean self-test failed.'}
    if([math]::Abs($sd-1.0) -gt 1e-12){throw 'Stat SD self-test failed.'}
    $rows=@(
    ...
    $cut='2025-04-02'
    $train=@($rows|Where-Object{[string]::CompareOrdinal([string]$_.response_day_utc,$cut) -lt 0})
    $test=@($rows|Where-Object{[string]::CompareOrdinal([string]$_.response_day_utc,$cut) -ge 0})
    if(($train.Count -ne 1) -or ($test.Count -ne 3)){throw 'Chronological split self-test failed.'}
...
try {
    ...
    foreach($marker in @('STAGE7_ACTIVE','CFA-S7-001','CFA-S7-002','MODEL_ELIGIBILITY_UNVERIFIED')){
        if($contract -notmatch [regex]::Escape($marker)){throw "Stage 7 contract marker missing: $marker"}
    }
    ...
    foreach($i in 1..8){
        $id=('CFA-S6-{0:D3}' -f $i)
        $gate=$s6.gates.PSObject.Properties[$id]
        if(($null -eq $gate) -or ([string]$gate.Value -ne 'PASS')){throw "Stage 6 prerequisite gate is not PASS: $id"}
    }
    if([long]$s6.violations.count -ne 0){throw 'Stage 6 receipt contains blocking violations.'}
    ...
    $factorSeen=New-Object 'Collections.Generic.HashSet[string]' ([StringComparer]::Ordinal)
    $patterns=@{}
    $dayAgg=@{}
    $assetAgg=@{}
    ...
    foreach($f in $factors){
        ...
        $key=$base+'|'+$day
        if(-not $factorSeen.Add($key)){throw "Duplicate Stage 5 factor key: $key"}
        ...
        if(-not $patterns.ContainsKey($pattern)){
            $patterns[$pattern]=[pscustomobject]@{
                rows=[long]0
                bases=(New-Object 'Collections.Generic.HashSet[string]' ([StringComparer]::Ordinal))
                days=(New-Object 'Collections.Generic.HashSet[string]' ([StringComparer]::Ordinal))
            }
        }
        $patternState=$patterns[$pattern]
        $patternState.rows=([long]$patternState.rows)+1
        [void]$patternState.bases.Add($base)
        [void]$patternState.days.Add($day)

        if(-not $dayAgg.ContainsKey($day)){
            $dayAgg[$day]=[pscustomobject]@{
                response_rows=[long]0
                market_available=[long]0
                news24_available=[long]0
                news6_available=[long]0
                all7_eligible=[long]0
                eligible_bases=(New-Object 'Collections.Generic.HashSet[string]' ([StringComparer]::Ordinal))
            }
        }
        $dayState=$dayAgg[$day]
        $dayState.response_rows=([long]$dayState.response_rows)+1
        if($marketDefined){$dayState.market_available=([long]$dayState.market_available)+1}
        if($news24Defined){$dayState.news24_available=([long]$dayState.news24_available)+1}
        if($news6Defined){$dayState.news6_available=([long]$dayState.news6_available)+1}

        if(-not $assetAgg.ContainsKey($base)){$assetAgg[$base]=[pscustomobject]@{response_days=[long]0;all7_days=[long]0;first_all7='';last_all7=''}}
        $assetState=$assetAgg[$base]
        $assetState.response_days=([long]$assetState.response_days)+1

        if($all7){
            $dayState.all7_eligible=([long]$dayState.all7_eligible)+1
            [void]$dayState.eligible_bases.Add($base)
            $assetState.all7_days=([long]$assetState.all7_days)+1
            $firstText=[string]$assetState.first_all7
            $lastText=[string]$assetState.last_all7
            if([string]::IsNullOrWhiteSpace($firstText) -or ([string]::CompareOrdinal($day,$firstText) -lt 0)){$assetState.first_all7=$day}
            if([string]::IsNullOrWhiteSpace($lastText) -or ([string]::CompareOrdinal($day,$lastText) -gt 0)){$assetState.last_all7=$day}
    ...
    if($responseByKey.Count -ne $factorSeen.Count){throw 'Response/factor key cardinality mismatch.'}
    $eligibleRows=@($eligible.ToArray()|Sort-Object response_day_utc,base_asset_id)
    if($eligibleRows.Count -lt 1){throw 'No all-seven-eligible rows observed.'}
    $eligibleBases=@($eligibleRows|Select-Object -ExpandProperty base_asset_id|Sort-Object -Unique)
    $eligibleDays=@($eligibleRows|Select-Object -ExpandProperty response_day_utc|Sort-Object -Unique)
    ...
    for($i=1;$i -lt $eligibleDays.Count;$i++){
        ...
        $train=@($eligibleRows|Where-Object{[string]::CompareOrdinal([string]$_.response_day_utc,$cut) -lt 0})
        $test=@($eligibleRows|Where-Object{[string]::CompareOrdinal([string]$_.response_day_utc,$cut) -ge 0})
        $trainDays=@($train|Select-Object -ExpandProperty response_day_utc|Sort-Object -Unique)
        $testDays=@($test|Select-Object -ExpandProperty response_day_utc|Sort-Object -Unique)
        $trainAssets=@($train|Select-Object -ExpandProperty base_asset_id|Sort-Object -Unique)
        $testAssets=@($test|Select-Object -ExpandProperty base_asset_id|Sort-Object -Unique)
    ...
    $receipt=[ordered]@{
        status='PASS'
        stage='CFA_STAGE_7'
        diagnostic='MODEL_ELIGIBILITY_V1'
