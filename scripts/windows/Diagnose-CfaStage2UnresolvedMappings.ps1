...
function Build-Diagnostic{
 ...
 foreach($d in $unresolved){
   ...
   $rows+=[pscustomobject]@{base_asset_id=$base;base_exchange_symbols=($exchangeSymbols-join'|');ws_base_symbols=($wsBaseSymbols-join'|');candidate_count=$candidateCount;candidate_ids=($candidateIds-join'|');diagnostic_shape=$shape;exchange_vs_ws_symbol_mismatch=$symbolMismatch;eligible_pair_rows=$pairs.Count;quote_symbols=($quotes-join'|');official_pair_key_count=$pairKeys.Count;typed_observations=$obs;prior_reason=[string]$d.decision_reason}
 }
...
function Invoke-SelfTest{$d=@([pscustomobject]@{base_asset_id='A';candidate_ids='x';mapping_status='UNVERIFIED';decision_reason='x'},[pscustomobject]@{base_asset_id='B';candidate_ids='';mapping_status='UNVERIFIED';decision_reason='x'});$p=@([pscustomobject]@{base_asset_id='A';base_exchange_symbol='AA';official_wsname='AA/USD';quote_exchange_symbol='USD';official_pair_key='AUSD';typed_observation_count=5});$r=Build-Diagnostic $d $p;if($r.Count-ne2-or@($r|Where-Object{$_.diagnostic_shape-eq'SINGLE_CANDIDATE_NO_CURRENT_BRIDGE'}).Count-ne1){throw 'diagnostic self-test'};Write-Host 'SELF-TEST: PASS'}
