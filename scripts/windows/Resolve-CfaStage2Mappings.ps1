...
function Resolve-Decision{
 ...
 if($null-ne$Adj){
   ...
   if([string]$Adj.decision_status-eq'NOT_APPLICABLE'){return [pscustomobject]@{status='NOT_APPLICABLE';id='';basis=[string]$Adj.evidence_basis;evidence=([string]$Adj.review_note+' Source: '+[string]$Adj.evidence_source_url)}}
   return [pscustomobject]@{status='APPROVED';id=[string]$Adj.approved_coingecko_id;basis=[string]$Adj.evidence_basis;evidence=([string]$Adj.review_note+' Source: '+[string]$Adj.evidence_source_url)}
 }
...
function Invoke-SelfTest{
 ...
 $adj=[pscustomobject]@{decision_status='APPROVED';approved_coingecko_id='abc';expected_candidate_name='Asset ABC';expected_candidate_symbol='abc';evidence_basis='TEST';evidence_source_url='https://example.invalid';observed_kraken_name='Asset ABC';observed_kraken_ticker='ABC';review_note='test'}
 $r=Resolve-Decision $row $adj $candidate;if($r.status-ne'APPROVED'-or$r.id-ne'abc'){throw 'adjudication approval'}
...
try{
 ...
 $adjPath=Join-Path $RepoRoot 'candidate-analysis\CFA-Stage2-Mapping-Adjudications.csv';$adjs=@(Import-Csv -LiteralPath $adjPath);$adjByBase=@{};foreach($a in $adjs){$b=[string]$a.base_asset_id;if($adjByBase.ContainsKey($b)){throw "Duplicate mapping adjudication: $b"};$adjByBase[$b]=$a}
