...
function Invoke-SelfTest {
    ...
    $raw=[pscustomobject]@{page_title='New Cryptocurrency Releases - Grade, IP Exchange, AMALAS';structured_names=@()};$row=[pscustomobject]@{};$meta=[pscustomobject]@{short_aliases=@('IP');approved_nondefault=$false};$c=Classify-V6 $row $meta $raw;if([bool]$c.keep){throw'IP Exchange V6 regression must reject.'}
    ...
    $raw=[pscustomobject]@{page_title='Any title';structured_names=@('IP')};$meta=[pscustomobject]@{short_aliases=@('IP');approved_nondefault=$false};$c=Classify-V6 $row $meta $raw;if(-not[bool]$c.keep){throw'Exact structured short symbol must retain.'}
